...
const Stack = () => {
  return (
    <div className="relative w-full overflow-hidden rounded-[2rem] border border-white/5 bg-slate-900/40 p-5 sm:p-8 shadow-2xl backdrop-blur-sm">
      {/* Decorative glows */}
      <div className="pointer-events-none absolute top-0 right-0 h-40 w-40 bg-emerald-500/5 rounded-full blur-[80px]" />
      <div className="pointer-events-none absolute bottom-0 left-0 h-40 w-40 bg-indigo-500/5 rounded-full blur-[80px]" />

      <div className="relative z-10 mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-base sm:text-xl font-black text-white tracking-tight">Revenue Breakdown</h3>
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Vehicle Category Distribution</p>
        </div>
        <div className="flex items-center gap-1.5 rounded-2xl border border-white/5 bg-white/5 px-3 py-1.5">
      ...
      {/* Explicit height container so ResponsiveContainer always has valid dimensions */}
      <div className="h-[260px] sm:h-[320px] lg:h-[360px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "#475569", fontSize: 10, fontWeight: 800 }} dy={12} />
            ...
            />
            <Area type="monotone" dataKey="SUV"    stackId="1" stroke="#10b981" strokeWidth={3} fill="url(#colorSUV)"    animationDuration={2500} />
            <Area type="monotone" dataKey="Sedan"  stackId="1" stroke="#22d3ee" strokeWidth={3} fill="url(#colorSedan)"  animationDuration={2500} />
            <Area type="monotone" dataKey="Luxury" stackId="1" stroke="#6366f1" strokeWidth={3} fill="url(#colorLuxury)" animationDuration={2500} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
